...
var Beer = function () {
	function Beer(name, description, tagline, image, abv, ibu, ebc, ph, hops, hops1, hops2, malt, malt1, malt2, yeast) {
	...
	_createClass(Beer, [{
		...
		value: function addToPage() {
			...
			var container = document.getElementById('beerlist');
...
function quickfind() {
	...
	_axios2.default.get('https://api.punkapi.com/v2/beers/random').then(function (response) {
		...
		try {
			for (var _iterator2 = beerData[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
...
var pickbeerlink = document.querySelector('.pickbeerlink');
		...
		try {
			for (var _iterator3 = beerData[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
				...
				if (beerData.length) {
					...
					var malt = value.ingredients.malt[0].name;
					var malt1 = value.ingredients.malt[0].amount.value;
					var malt2 = value.ingredients.malt[0].amount.unit;
					var hops = value.ingredients.hops[0].name;
					var hops1 = value.ingredients.hops[0].amount.value;
